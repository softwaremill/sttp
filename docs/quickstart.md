...
`sttp client` is also available for Scala.js 1.0. Note that not all modules are compatible and there are no backends that can be used on both. The last version compatible with Scala.js 0.6 was 2.1.4.
...
```scala
import $ivy.`com.softwaremill.sttp.client::okhttp-backend:2.1.4`
