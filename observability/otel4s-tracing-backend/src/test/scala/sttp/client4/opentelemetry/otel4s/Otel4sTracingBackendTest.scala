...
class Otel4sTracingBackendTest extends AnyFlatSpec with Matchers {

  "Otel4sTracingBackend" should "add tracing headers to the request" in {
    TracesTestkit
  ...
  it should "record request/response-specific attributes: 200 OK response" in {
    TracesTestkit
      .inMemory[IO](_.addTextMapPropagators(W3CTraceContextPropagator.default))
      .use { testkit =>
        implicit val tracerProvider: TracerProvider[IO] = testkit.tracerProvider
  ...
  it should "record request/response-specific attributes: 400 BadRequest response" in {
    TracesTestkit
      .inMemory[IO](_.addTextMapPropagators(W3CTraceContextPropagator.default))
      .use { testkit =>
        implicit val tracerProvider: TracerProvider[IO] = testkit.tracerProvider
  ...
  it should "record request/response-specific attributes: runtime error" in {
