...
object SttpClientException {
  ...
  def adjustSynchronousExceptions[T](t: => T)(using: Exception => Option[Exception]): T = {
    try t
    catch {
      case e: Exception => throw using(e).getOrElse(e)
    }
  }

  def adjustExceptions[F[_], T](
      monadError: MonadError[F]
  )(t: => F[T])(using: Exception => Option[Exception]): F[T] = {
    monadError.handleError(t) {
      case e: Exception => monadError.error(using(e).getOrElse(e))
    }
  }
}
