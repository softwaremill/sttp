...
/** Describes a partial HTTP request, along with a description of how the response body should be handled. A partial
  * request cannot sent because the method and uri are not yet specified.
  *
  * @param response
...
final case class PartialRequest[T](
  ...
  override protected def withOptions(options: RequestOptions): PartialRequest[T] = copy(options = options)
  override protected def copyWithBody(body: BasicBody): PartialRequest[T] = copy(body = body)
  override protected def withTags(tags: Map[String, Any]): PartialRequest[T] = copy(tags = tags)
