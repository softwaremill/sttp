...
/**
  ...
  * headers and status code. Responses can also be handled depending on the response metadata. Finally, two response
  * body descriptions can be combined (with some restrictions).
  *
  ...
  * @tparam R The backend capabilities required by the response description. This might be `Any` (no
