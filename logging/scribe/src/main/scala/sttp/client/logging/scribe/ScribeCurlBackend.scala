...
object ScribeCurlBackend {
  def apply[F[_], S, WS_HANDLER[_]](delegate: SttpBackend[F, S, WS_HANDLER]): SttpBackend[F, S, WS_HANDLER] =
    ListenerBackend.lift(delegate, new ScribeCurlListener())
}

class ScribeCurlListener() extends RequestListener[Identity, Unit] {
  ...
  override def requestException(request: Request[_, _], tag: Unit, e: Exception): Identity[Unit] = {
    scribe.debug(LogMessages.requestCurl(request, "exception"), e)
  }

  override def requestSuccessful(request: Request[_, _], response: Response[_], tag: Unit): Identity[Unit] = {
    scribe.debug(LogMessages.requestCurl(request, response.code.toString()))
  }
  ...
  override def websocketSuccessful(
    ...
    scribe.debug(LogMessages.requestCurl(request, "websocket"))
  }
}
