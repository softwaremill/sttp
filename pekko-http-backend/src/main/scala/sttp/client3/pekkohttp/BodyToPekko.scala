...
private[pekkohttp] object BodyToPekko {
  def apply[R](
    ...
    def toBodyPart(mp: Part[RequestBody[_]]): Try[PekkoMultipart.FormData.BodyPart] = {
      ...
      for {
        ...
        PekkoMultipart.FormData.BodyPart(mp.name, entity(ct), mp.dispositionParams, headers)
      }
    }
  ...
  private def multipartEntity(r: Request[_, _], bodyParts: Seq[PekkoMultipart.FormData.BodyPart]): Try[RequestEntity] = {
